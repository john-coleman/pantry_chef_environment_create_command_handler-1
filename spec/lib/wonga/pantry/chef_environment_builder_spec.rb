  ...
  describe "#build!" do
    context "when creating a CI environment" do
      ...
      context "creates new chef_environment" do
        ...
        %w( authorization build_agent build-essential java jenkins iis nginx openssh postfix snmp wonga wonga_windows wonga_splunk wonga_winbind ).each do |group|
    ...
    context "when creating a STG environment" do
      ...
      context "creates new chef_environment" do
        ...
        %w( authorization build_agent build-essential drush frontend_server iis newrelic nginx openssh postfix snmp wonga wonga_windows wonga_splunk wonga_winbind ).each do |group|
          it "with #{group} information" do
            expect(environment.default_attributes).to have_key(group)
          end
        end
